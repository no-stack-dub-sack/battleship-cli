...
class Player {
    ...
    fireTorpedo(coords, opponent, cpu) {
        ...
        switch (SPACE_ATTACKED) {
            ...
            case ' 💥 ':
                return {
                    shipHit: false,
                    message: LOST_TURN + BOARD_UPDATE
                }
    ...
    updateShip(abbreviation, cpu) {
        const YOU_SUNK = '\nYou bastard! You sunk the computer\'s ';
        const CPU_SUNK = '\nHa ha! The computer sunk your ';
        const CPU_HIT = '\nUh oh. The computer hit your ';
        const YOU_HIT = '\nYou hit a ship!';
    ...
    validateInstruction(ship, coords, direction) {
        ...
        for (let __ship of this.ships) {
            if ((__ship.type === ship || __ship.abbreviation === ship) && __ship.placed) {
                return {
                    isValid: false,
